// Package cli builds the mo command tree on top of the mathoverflow library.
package cli
...
type App struct {
	client *mathoverflow.Client
	cfg    mathoverflow.Config
...
func Root() *cobra.Command {
	app := &App{cfg: mathoverflow.DefaultConfig()}

	root := &cobra.Command{
		Use:   "mo",
		Short: "Browse MathOverflow research questions",
		Long: `mo reads MathOverflow through the public Stack Exchange API v2.3.
No API key is required. It returns records as table, JSON, JSONL,
CSV, TSV, or URLs.

mo is an independent tool and is not affiliated with MathOverflow or Stack Exchange.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = mathoverflow.NewClient(a.cfg)
	return nil
}
