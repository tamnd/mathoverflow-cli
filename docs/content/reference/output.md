...
describes what users get. Pick a format with `-o`, or let mo choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
mo <command> -o table   # aligned columns for reading
mo <command> -o jsonl   # one JSON object per line, for piping
mo <command> -o json    # a single JSON array
mo <command> -o csv     # spreadsheet friendly
mo <command> -o tsv     # tab-separated
mo <command> -o url     # just the URL column
mo <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
mo <command> --fields id,title,url
```
...
```bash
mo <command>            # a table, because this is a terminal
mo <command> | wc -l    # JSONL, because this is a pipe
```
